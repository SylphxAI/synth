...
Pull requests and merge groups run `.github/workflows/ci.yml`, including ADR-29 classification/fan-in contexts, isolated Rust setup, `bun run validate`, and behavior/parity tests. Main pushes run postsubmit proof and `.github/workflows/release.yml` for Changesets release PRs or package publication. Published package changes require Release workflow evidence and npm registry readback.
