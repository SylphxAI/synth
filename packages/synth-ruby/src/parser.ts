...
let _rubyLanguage: Language | null = null
...
async function initParser(): Promise<Parser> {
  ...
  parserPromise = (async () => {
    ...
    // Load Ruby language from tree-sitter-wasms
    ...
    try {
      ...
      try {
        ...
        const wasmFile = require.resolve('tree-sitter-wasms/out/tree-sitter-ruby.wasm')
        language = await Parser.Language.load(wasmFile)
      } catch {
        // Fallback: try import from package directly
        const { default: rubyWasm } = await import('tree-sitter-wasms/out/tree-sitter-ruby.wasm')
        language = await Parser.Language.load(rubyWasm)
      }
    }

    _rubyLanguage = language
...
export class RubyParser {
   ...
   */
  parse(_source: string, _options: RubyParseOptions = {}): Tree {
 ...
 * @deprecated Use parseAsync() instead - WASM requires async initialization
 */
export function parse(_source: string, _options?: RubyParseOptions): Tree {
  throw new SynthError(
    'Synchronous parse() is not supported with WASM. Use parseAsync() instead.',
    'SYNC_NOT_SUPPORTED'
  )
...
export async function parseAsync(source: string, options?: RubyParseOptions): Promise<Tree> {
