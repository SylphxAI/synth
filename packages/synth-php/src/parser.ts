/**
 * PHP Parser (WASM-based)
 *
 * Converts PHP source to Synth's universal AST
...
import type { PhpParseOptions } from './types.js'

// Singleton parser instance (reused across calls)
let parserPromise: Promise<Parser> | null = null
let _phpLanguage: Language | null = null
...
async function initParser(): Promise<Parser> {
  ...
  parserPromise = (async () => {
    ...
    // Load PHP language from tree-sitter-wasms
    ...
    try {
      ...
      try {
        ...
        const wasmFile = require.resolve('tree-sitter-wasms/out/tree-sitter-php.wasm')
        language = await Parser.Language.load(wasmFile)
      } catch {
        // Fallback: try import from package directly
        const { default: phpWasm } = await import('tree-sitter-wasms/out/tree-sitter-php.wasm')
        language = await Parser.Language.load(phpWasm)
      }
    }

    _phpLanguage = language
...
export class PhpParser {
  ...
  /**
   * Parse PHP synchronously
   * @deprecated Use parseAsync() instead - WASM requires async initialization
   */
  parse(_source: string, _options: PhpParseOptions = {}): Tree {
  ...
  /**
   * Parse PHP asynchronously
   */
  async parseAsync(source: string, options: PhpParseOptions = {}): Promise<Tree> {
    // Initialize parser if needed
    if (!this.parser) {
      this.parser = await initParser()
    }

    const tree = createTree('php', source)
    this.tree = tree
    ...
    try {
      ...
      throw new SynthError(`PHP parse error: ${error}`, 'PARSE_ERROR')
    }
...
export function createParser(): PhpParser {
 ...
 * @deprecated Use parseAsync() instead - WASM requires async initialization
 */
export function parse(_source: string, _options?: PhpParseOptions): Tree {
  throw new SynthError(
    'Synchronous parse() is not supported with WASM. Use parseAsync() instead.',
    'SYNC_NOT_SUPPORTED'
  )
